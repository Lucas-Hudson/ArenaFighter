class Fighter < ApplicationRecord
  ...
  def attacks(fighter)
    if self.lifepoints > 0
      ...
      store_message("#{self.name} attacks #{fighter.name}")
      # Calculate the damage
      damage_inflicted = self.hitpoints + compute_damage
      # Store message of the damage inflicted
      store_message("Damage is #{damage_inflicted}")
  ...
  def gets_damage(fighter, damage)
    ...
    # Store message
    store_message("#{fighter.name} has #{fighter.lifepoints} lifepoints left")
